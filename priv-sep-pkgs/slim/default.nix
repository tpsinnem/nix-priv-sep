...
stdenv.mkDerivation rec {
  ...
  patches =
    [ # Instead of a) first switching the process uid & gid to the login user and 
      # then writing the .Xauthority file, we b) write .Xauthority (as root), chown 
      # the file to the user, and then switch the process uid & gid. This is to 
      # allow setting .Xauthority even for users who have no write access to their 
      # home directory.
      ./chown-xauthority.patch
